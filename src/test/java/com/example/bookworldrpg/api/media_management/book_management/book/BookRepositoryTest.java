...
public class BookRepositoryTest {
  ...
  public void shouldFindTitleByUniqueConstraint() {
    ...
    BookEntity book = BookEntity.builder().title("Book1").author(author).genre(genre).build();
    ...
    Optional<BookEntity> result =
        bookRepository.findByTitleAndGenreIdAndAuthorId(
            book.getTitle(), genre.getId(), author.getId());
  ...
  public void shouldReturnEmptyOptionalIfBookNotFoundByUniqueConstraint() {
    // Given / When
    Optional<BookEntity> result =
        bookRepository.findByTitleAndGenreIdAndAuthorId("Title", 100L, 100L);
